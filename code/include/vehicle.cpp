...
bool vehicle::process_event(event event_obj, double &next_time, double &time_left){
    ...
    //------ code for fairness
    double curr_time = next_time - time_left;
    //------ code for fairness
    // Pick Up
    if (event_obj.type == 0){
        if (VERBOSITY == -1){
        ...
        if (next_time > food_prepared_time) {
            ...
            //------ code for fairness
            this->wait_time += max((double)(food_prepared_time-curr_time), double(0.0));
            //------ code for fairness
...
bool vehicle::move(double curr_time, double next_time){
    ...
    if ((this->route_plan).empty()){
        ...
        //------ code for fairness
        if (this->ONLINE == true){
            this->idle_time +=  time_left;
            if (VERBOSITY == -1)
                cout << "IDLE,"<< this->vehicle_id << ","<< next_time - time_left << "," <<  time_left << endl;
    ...
    while (!(this->route_plan).empty() && time_left > 0){
        ...
        // Move on Full Edge
            ...
            //------ code for fairness
            this->travel_time +=  time_to_next_node * (1 - this->fraction_last_edge);
            //------ code for fairness
